...
1. `account_ops`：先稳定采集。
2. `shop_account_analysis`：再做账号深度分析。
3. `merchant_brain_factory`：再创建商家独立大脑。
4. `content_pipeline`：后续根据商家素材生成内容方向。
5. `data_review`：后续根据发布数据复盘调整。
...
当前只正式运行 `modules/account_ops` 账号采集模块。下一步是建设 `modules/shop_account_analysis`，再下一步才是 `modules/merchant_brain_factory` 和 `merchants/` 商家独立大脑体系。
