...
任何 AI / Codex 进入本项目必须先读取 `STATE.json`、`STATE_CONSISTENCY_LOCK.md` 和 `SEMANTIC_LAYERS.md`。

语义分层协议要求：

- `MASTER_CONTROL.md` 只负责执行权限。
- `PROJECT_FRAMEWORK.md` 只负责系统设计。
- `STATE.json` 只负责当前真实状态。
- `TASKS.json` 只负责下一步动作。
...
4. `ENTRY_PROTOCOL.md`
5. `STATE_CONSOLIDATION_RULES.md`
6. `MASTER_CONTROL.md`
