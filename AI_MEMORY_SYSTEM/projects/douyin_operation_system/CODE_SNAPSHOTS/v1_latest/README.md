...
`project_brain` 与 `douyin_account_ops` 已合并为 `douyin_operation_system`。历史源目录仍保留在 `projects/` 下作为历史材料，但 registry 入口以 `01_PROJECT_REGISTRY/index.json` 为准。
...
`00_GLOBAL_MEMORY`、`01_PROJECT_REGISTRY`、`02_PROJECT_FACTORY` 等既有系统层文件保留为历史结构和容器辅助文件；v2.0 核心定位是外部记忆容器，不直接承载业务逻辑。

## 状态收敛提示

README 只作为外部大脑入口说明，不作为状态源。

- 状态判断以 `AI_MEMORY_SYSTEM/projects/douyin_operation_system/STATE.json` 为准。
- 任务判断以 `AI_MEMORY_SYSTEM/projects/douyin_operation_system/TASKS.json` 为准。
- 事实记录以 `AI_MEMORY_SYSTEM/projects/douyin_operation_system/LOGS.md` 为准。
- 冲突处理按 `STATE_CONSOLIDATION_RULES.md` 执行。
